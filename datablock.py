...
def procdata(ddd,
    ...
    if use_tsfel:
        ...
        for i in tqdm(range(len(data) - dwinlen)):
            ...
            dw.append(X.values)
        dw = np.vstack(dw)
        cs = ['X' + uchar + 'TSFL' + str(i) for i in range(dw.shape[1])]
    ...
    if not use_forex: addx(
        ta.aobv(close, volume, fast=None, slow=None, mamode=None, max_lookback=None, min_lookback=None,
                offset=None))
